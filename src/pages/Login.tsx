...
function Login() {
    ...
    //Fetch token balance
    const { balance, isFetched: isfetchTokenBalance } = useERC20Balance(constants().spaceCrownTokenInfo.address, constants().fetchBalanceWalletAddress)
                            ...
                            <div className={`flex flex-col mr-2 ${isMobileDevice ? "mt-6 mr-0" : 'mt-10'}`}>
                                ...
                                <IonButton className='mb-4 h-11' color={mode === 'dark' ? '' : "dark"}
                                    onClick={() => {
                                        ...
                                        const urlToRedirect = `https://discord.com/api/oauth2/authorize?client_id=${environment.clientId
                                ...
                                <IonButton className='buy-nft-btn mt-4 h-11' color='medium' onClick={() => window.open('https://magiceden.io/marketplace/soldecoder', "_blank")}>
                                    ...
                                    {
                                        !isfetchTokenBalance ? "Buy 1 NFT to gain access" : `${ethers.formatUnits(balance, constants().spaceCrownTokenInfo.decimals)} ${constants().spaceCrownTokenInfo.symbol}`
                                    }
